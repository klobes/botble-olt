...
@section('content')
        ...
        <div class="col-md-12">
            <div class="page-header">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h1>{{ trans('plugins/fiberhome-olt-manager::olt.create') }}</h1>
                        <p class="text-muted">{{ trans('plugins/fiberhome-olt-manager::olt.create_description') }}</p>
                    </div>
                    <div>
                        <a href="{{ route('fiberhome.olt-devices.index') }}" class="btn btn-secondary">
    ...
    <div class="row">
        <div class="col-md-12">
            <form action="{{ route('fiberhome.olt-devices.store') }}" method="POST" id="olt-form">
                @csrf
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('model') is-invalid @enderror" 
                                        ...
                                        <option value="AN5516-01" {{ old('model') === 'AN5516-01' ? 'selected' : '' }}>AN5516-01</option>
                                        <option value="AN5516-04" {{ old('model') === 'AN5516-04' ? 'selected' : '' }}>AN5516-04</option>
                                        <option value="AN5516-06" {{ old('model') === 'AN5516-06' ? 'selected' : '' }}>AN5516-06</option>
                                        <option value="AN6000-17" {{ old('model') === 'AN6000-17' ? 'selected' : '' }}>AN6000-17</option>
                                    </select>
                                    @error('model')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" 
                                           ...
                                           value="{{ old('polling_interval', 300) }}" 
                                           min="60" 
                                           max="3600">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('status') is-invalid @enderror" 
                                            id="status" 
                                            name="status">
                                        <option value="online" {{ old('status', 'online') === 'online' ? 'selected' : '' }}>
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> {{ trans('plugins/fiberhome-olt-manager::olt.save') }}
                            </button>
                        </div>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#test-connection').on('click', function() {
            const btn = $(this);
            const ipAddress = $('#ip_address').val();
            const snmpPort = $('#snmp_port').val();
            const snmpVersion = $('#snmp_version').val();
            const snmpCommunity = $('#snmp_community').val();

            if (!ipAddress || !snmpCommunity) {
                toastr.error('{{ trans('plugins/fiberhome-olt-manager::olt.fill_required') }}');
                return;
            }

            btn.prop('disabled', true);
            btn.html('<i class="fas fa-spinner fa-spin"></i> {{ trans('plugins/fiberhome-olt-manager::olt.testing') }}');

            $.ajax({
                url: '{{url("admin/fiberhome/olt-devices")}}/'+oltId+'/test-connection',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                ...
                error: function() {
                    toastr.error('{{ trans('plugins/fiberhome-olt-manager-manager::olt.connection_error') }}');
                },
                complete: function() {
                    btn.prop('disabled', false);
                    btn.html('<i class="fas fa-plug"></i> {{ trans('plugins/fiberhome-olt-manager::olt.test_connection') }}');
                }
            });
        });
    });
